...
if __name__ == '__main__': 
    ...
    # Load trajectories
    time_np = np.load("c2h4_traj_all.npy", allow_pickle=True).item()   # Python dic. (.npy) {N_traj: (Nt,)} 
    T_np    = np.load("c2h4_T_all.npy", allow_pickle=True).item()   # Python dic. (.npy) {N_traj: (Nt,)} 
    P_np    = np.load("c2h4_P_all.npy", allow_pickle=True).item()   # Python dic. (.npy) {N_traj: (Nt,)} 
    Y_np    = np.load("c2h4_Y_all.npy", allow_pickle=True).item()   # Python dic. (.npy) {N_traj: (Nt, Ns)} 

    # Load end time index
    idx_target = np.load("c2h4_index_all.npy") # Numpy arr. (.npy) (N_traj)
